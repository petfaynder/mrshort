    ...
    <div class="mx-auto w-full px-4 py-8 sm:px-6 md:px-8">
        <div class="flex flex-col gap-8">
            <div class="flex flex-col flex-wrap justify-between gap-4 sm:flex-row sm:items-center">
                ...
                <p class="text-3xl font-black leading-tight tracking-[-0.033em] text-gray-900 dark:text-white">Welcome back, <?php echo e(Auth::user()->name); ?></p>
            ...
            <div class="flex flex-wrap gap-4">
                <div class="flex min-w-[158px] flex-1 flex-col gap-2 rounded-xl border border-gray-200 bg-white p-6 dark:border-gray-800 dark:bg-gray-900/50">
                    ...
                    <p class="tracking-light text-2xl font-bold leading-tight text-gray-900 dark:text-white">$<?php echo e(number_format((Auth::user()->link_earnings ?? 0) + (Auth::user()->referral_earnings ?? 0), 2)); ?></p>
                </div>
            </div>
        </div>
        <div class="rounded-xl border border-gray-200 bg-white p-4 dark:border-gray-800 dark:bg-gray-900/50 sm:p-6">
            ...
            <form wire:submit.prevent.stop="shortenLink" class="flex flex-col gap-3 sm:flex-row sm:items-end">
                <label class="flex flex-col flex-1">
                    ...
                    <input type="url" wire:model="original_url" id="original_url" class="form-input flex w-full min-w-0 flex-1 resize-none overflow-hidden rounded-lg border border-gray-300 bg-gray-50 text-gray-900 placeholder:text-gray-400 focus:border-primary focus:ring-primary dark:border-gray-700 dark:bg-background-dark dark:text-white dark:placeholder:text-gray-500 h-14 p-[15px] text-base font-normal leading-normal <?php $__errorArgs = ['original_url'];
...
unset($__errorArgs, $__bag); ?>" placeholder="https://enter-a-long-url-to-shorten.com/..." />
                </label>
...
unset($__errorArgs, $__bag); ?><!--[if ENDBLOCK]><![endif]-->
        ...
        <div class="flex flex-col gap-4">
            ...
            <!--[if BLOCK]><![endif]--><?php if(count($selectedLinks) > 0): ?>
            <div class="flex items-center justify-between rounded-lg border border-gray-200 bg-gray-50 px-4 py-2 dark:border-gray-800 dark:bg-gray-900">
                <span class="text-sm font-medium text-gray-600 dark:text-gray-300"><?php echo e(count($selectedLinks)); ?> links selected</span>
...
<?php $component = Illuminate\View\AnonymousComponent::resolve(['view' => 'components.confirm-modal','data' => ['id' => 'delete-selected-links','title' => 'Delete Selected Links','message' => 'Are you sure you want to delete selected links? This action cannot be undone.','confirmText' => 'Delete All','cancelText' => 'Cancel','confirmColor' => 'red','icon' => 'delete']] + (isset($attributes) && $attributes instanceof Illuminate\View\ComponentAttributeBag ? $attributes->all() : [])); ?>
...
<?php endif; ?>
<?php $component->withAttributes(['id' => 'delete-selected-links','title' => 'Delete Selected Links','message' => 'Are you sure you want to delete selected links? This action cannot be undone.','confirmText' => 'Delete All','cancelText' => 'Cancel','confirmColor' => 'red','icon' => 'delete']); ?>
                            ...
                            <!--[if BLOCK]><![endif]--><?php if($links->count()): ?>
                                <!--[if BLOCK]><![endif]--><?php $__currentLoopData = $links; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $link): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                    <tr class="hover:bg-gray-50/50 dark:hover:bg-gray-900">
                                        <td class="w-4 p-4">
                                            <div class="flex items-center">
                                                <input wire:model.live="selectedLinks" value="<?php echo e($link->id); ?>" class="form-checkbox h-4 w-4 rounded border-gray-300 bg-gray-100 text-primary focus:ring-2 focus:ring-primary dark:border-gray-600 dark:bg-gray-700 dark:ring-offset-gray-800" id="checkbox-<?php echo e($link->id); ?>" type="checkbox"/>
                                                <label class="sr-only" for="checkbox-<?php echo e($link->id); ?>">select link <?php echo e($link->id); ?></label>
                                            </div>
                                        </td>
                                        <td class="whitespace-nowrap px-6 py-4">
                                            <div class="group relative flex items-center gap-2">
                                                <a class="text-primary hover:underline" href="<?php echo e($link->shortLink()); ?>" target="_blank"><?php echo e($link->shortLink()); ?></a>
                                                <button onclick="copyToClipboard('<?php echo e($link->shortLink()); ?>')" class="text-gray-500 hover:text-primary dark:text-gray-400 dark:hover:text-primary" data-alt="Copy link button">
                                                ...
                                                <div class="pointer-events-none absolute -top-1 left-0 z-10 w-max -translate-y-full rounded-md bg-gray-900 px-3 py-1.5 text-xs text-white opacity-0 transition-opacity group-hover:opacity-100 dark:bg-gray-700">
                                                    Preview: <?php echo e(Str::limit($link->original_url, 30)); ?>

                                                    <div class="absolute left-4 top-full h-2 w-2 rotate-45 bg-gray-900 dark:bg-gray-700"></div>
                                                </div>
                                            </div>
                                        </td>
                                        <td class="max-w-xs truncate px-6 py-4 font-medium text-gray-900 dark:text-white" title="<?php echo e($link->original_url); ?>"><a class="hover:underline" href="<?php echo e($link->original_url); ?>" target="_blank"><?php echo e(Str::limit($link->original_url, 50)); ?></a></td>
                                        <td class="whitespace-nowrap px-6 py-4 text-center"><?php echo e($link->clicks); ?></td>
                                        <td class="px-6 py-4">
                                            <div class="flex h-8 w-24 items-end gap-0.5" x-data="{
                                                stats: <?php echo e(json_encode($performanceData[$link->id] ?? array_fill(0, 7, 0))); ?>,
                                                get maxClicks() {
                                        ...
                                        <td class="whitespace-nowrap px-6 py-4 text-gray-500 dark:text-gray-400"><?php echo e($link->created_at->format('M d, Y')); ?></td>
                                        <td class="whitespace-nowrap px-6 py-4">
                                            <div class="flex items-center justify-end gap-2">
                                                <button wire:click="toggleStats(<?php echo e($link->id); ?>)" class="flex items-center gap-1.5 rounded-md px-2.5 py-1.5 text-xs font-semibold text-gray-600 hover:bg-gray-100 dark:text-gray-300 dark:hover:bg-gray-800">
                                                    <span class="material-symbols-outlined text-sm">bar_chart</span> Stats
                                                </button>
                                                <button wire:click="editLink(<?php echo e($link->id); ?>)" class="flex items-center gap-1.5 rounded-md px-2.5 py-1.5 text-xs font-semibold text-gray-600 hover:bg-gray-100 dark:text-gray-300 dark:hover:bg-gray-800">
                                                    <span class="material-symbols-outlined text-sm">settings</span> Details
                                                </button>
                                                <button wire:click="hideLink(<?php echo e($link->id); ?>)" wire:loading.attr="disabled" class="flex items-center gap-1.5 rounded-md px-2.5 py-1.5 text-xs font-semibold text-orange-600 hover:bg-orange-50 dark:text-orange-500 dark:hover:bg-orange-500/10 disabled:opacity-75 disabled:cursor-not-allowed">
                                                    <span wire:loading.remove wire:target="hideLink(<?php echo e($link->id); ?>)" class="material-symbols-outlined text-sm">visibility_off</span>
                                                    <span wire:loading wire:target="hideLink(<?php echo e($link->id); ?>)" class="material-symbols-outlined text-sm animate-spin">progress_activity</span>
                                                    Hide
                                                </button>
                                                <button 
                                                    x-data
                                                    @click="$dispatch('open-confirm-modal', { 
                                                        id: 'delete-link-<?php echo e($link->id); ?>', 
                                                        onConfirm: () => $wire.deleteLink(<?php echo e($link->id); ?>) 
                                                    })"
                                                    wire:loading.attr="disabled" 
                                                    class="flex items-center gap-1.5 rounded-md px-2.5 py-1.5 text-xs font-semibold text-red-600 hover:bg-red-50 dark:text-red-500 dark:hover:bg-red-500/10 disabled:opacity-75 disabled:cursor-not-allowed"
                                                >
                                                    <span wire:loading.remove wire:target="deleteLink(<?php echo e($link->id); ?>)" class="material-symbols-outlined text-sm">delete</span>
                                                    <span wire:loading wire:target="deleteLink(<?php echo e($link->id); ?>)" class="material-symbols-outlined text-sm animate-spin">progress_activity</span>
                                                    Delete
                                                </button>
                                                <?php if (isset($component)) { $__componentOriginal2cfaf2d8c559a20e3495c081df2d0b10 = $component; } ?>
<?php if (isset($attributes)) { $__attributesOriginal2cfaf2d8c559a20e3495c081df2d0b10 = $attributes; } ?>
<?php $component = Illuminate\View\AnonymousComponent::resolve(['view' => 'components.confirm-modal','data' => ['id' => 'delete-link-'.e($link->id).'','title' => 'Delete Link','message' => 'Are you sure you want to delete this link?','confirmText' => 'Delete','cancelText' => 'Cancel','confirmColor' => 'red','icon' => 'delete']] + (isset($attributes) && $attributes instanceof Illuminate\View\ComponentAttributeBag ? $attributes->all() : [])); ?>
<?php $component->withName('confirm-modal'); ?>
<?php if ($component->shouldRender()): ?>
<?php $__env->startComponent($component->resolveView(), $component->data()); ?>
<?php if (isset($attributes) && $attributes instanceof Illuminate\View\ComponentAttributeBag): ?>
<?php $attributes = $attributes->except(\Illuminate\View\AnonymousComponent::ignoredParameterNames()); ?>
<?php endif; ?>
<?php $component->withAttributes(['id' => 'delete-link-'.e($link->id).'','title' => 'Delete Link','message' => 'Are you sure you want to delete this link?','confirmText' => 'Delete','cancelText' => 'Cancel','confirmColor' => 'red','icon' => 'delete']); ?>
<?php echo $__env->renderComponent(); ?>
<?php endif; ?>
<?php if (isset($__attributesOriginal2cfaf2d8c559a20e3495c081df2d0b10)): ?>
<?php $attributes = $__attributesOriginal2cfaf2d8c559a20e3495c081df2d0b10; ?>
...
<?php endif; ?>
                                    ...
                                    <!--[if BLOCK]><![endif]--><?php if($showingStats === $link->id): ?>
                                        <tr>
                                            <td colspan="8" class="px-6 py-4">
                                                <!--[if BLOCK]><![endif]--><?php if(!empty($statsData)): ?>
                                                    ...
                                                    <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-800">
                                                        ...
                                                        <tbody class="bg-white divide-y divide-gray-200 dark:divide-gray-800 dark:bg-gray-900/50">
                                                            <!--[if BLOCK]><![endif]--><?php $__currentLoopData = $statsData; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $stat): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                                                <tr>
                                                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900 dark:text-white">
                                                                        <?php echo e($stat->click_date); ?>

                                                                    </td>
                                                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900 dark:text-white">
                                                                        <?php echo e($stat->total_clicks); ?>
...
<!--[if BLOCK]><![endif]--><?php if($editingLink): ?>
                ...
                <div class="max-h-[70vh] space-y-6 overflow-y-auto p-6">
                    <div class="space-y-4">
                        ...
                        <div>
                            ...
                            <input wire:model="newOriginalUrl" class="form-input w-full rounded-lg border border-gray-300 bg-gray-50 text-sm text-gray-900 focus:border-primary focus:ring-primary dark:border-gray-700 dark:bg-gray-900 dark:text-white <?php $__errorArgs = ['newOriginalUrl'];
$__bag = $errors->getBag($__errorArgs[1] ?? 'default');
if ($__bag->has($__errorArgs[0])) :
if (isset($message)) { $__messageOriginal = $message; }
$message = $__bag->first($__errorArgs[0]); ?> border-red-500 <?php unset($message);
...
unset($__errorArgs, $__bag); ?>" id="original-url" type="text"/>
                            <!--[if BLOCK]><![endif]--><?php $__errorArgs = ['newOriginalUrl'];
$__bag = $errors->getBag($__errorArgs[1] ?? 'default');
if ($__bag->has($__errorArgs[0])) :
if (isset($message)) { $__messageOriginal = $message; }
...
unset($__errorArgs, $__bag); ?><!--[if ENDBLOCK]><![endif]-->
                        ...
                        <div>
                            ...
                            <div class="relative">
                                <input wire:model="newShortLink" class="form-input w-full rounded-lg border border-gray-300 bg-gray-50 pl-20 text-sm text-gray-900 focus:border-primary focus:ring-primary dark:border-gray-700 dark:bg-gray-900 dark:text-white <?php $__errorArgs = ['newShortLink'];
$__bag = $errors->getBag($__errorArgs[1] ?? 'default');
if ($__bag->has($__errorArgs[0])) :
if (isset($message)) { $__messageOriginal = $message; }
$message = $__bag->first($__errorArgs[0]); ?> border-red-500 <?php unset($message);
...
unset($__errorArgs, $__bag); ?>" id="short-link" type="text"/>
                            ...
                            <!--[if BLOCK]><![endif]--><?php $__errorArgs = ['newShortLink'];
$__bag = $errors->getBag($__errorArgs[1] ?? 'default');
if ($__bag->has($__errorArgs[0])) :
if (isset($message)) { $__messageOriginal = $message; }
$message = $__bag->first($__errorArgs[0]); ?> <span class="text-red-500 text-sm"><?php echo e($message); ?></span> <?php unset($message);
...
unset($__errorArgs, $__bag); ?><!--[if ENDBLOCK]><![endif]-->
                        ...
                        <div>
                            ...
                            <input wire:model="newTitle" class="form-input w-full rounded-lg border border-gray-300 bg-gray-50 text-sm text-gray-900 focus:border-primary focus:ring-primary dark:border-gray-700 dark:bg-gray-900 dark:text-white <?php $__errorArgs = ['newTitle'];
$__bag = $errors->getBag($__errorArgs[1] ?? 'default');
if ($__bag->has($__errorArgs[0])) :
if (isset($message)) { $__messageOriginal = $message; }
$message = $__bag->first($__errorArgs[0]); ?> border-red-500 <?php unset($message);
...
unset($__errorArgs, $__bag); ?>" id="link-title" placeholder="e.g. My Awesome Campaign" type="text"/>
                            <!--[if BLOCK]><![endif]--><?php $__errorArgs = ['newTitle'];
$__bag = $errors->getBag($__errorArgs[1] ?? 'default');
if ($__bag->has($__errorArgs[0])) :
if (isset($message)) { $__messageOriginal = $message; }
$message = $__bag->first($__errorArgs[0]); ?> <span class="text-red-500 text-sm"><?php echo e($message); ?></span> <?php unset($message);
...
unset($__errorArgs, $__bag); ?><!--[if ENDBLOCK]><![endif]-->
                        ...
                        <div>
                            ...
                            <input wire:model="newExpiresAt" class="form-input w-full rounded-lg border border-gray-300 bg-gray-50 text-sm text-gray-900 focus:border-primary focus:ring-primary dark:border-gray-700 dark:bg-gray-900 dark:text-white <?php $__errorArgs = ['newExpiresAt'];
$__bag = $errors->getBag($__errorArgs[1] ?? 'default');
if ($__bag->has($__errorArgs[0])) :
if (isset($message)) { $__messageOriginal = $message; }
$message = $__bag->first($__errorArgs[0]); ?> border-red-500 <?php unset($message);
...
unset($__errorArgs, $__bag); ?>" id="expires-at" type="datetime-local"/>
                            <!--[if BLOCK]><![endif]--><?php $__errorArgs = ['newExpiresAt'];
$__bag = $errors->getBag($__errorArgs[1] ?? 'default');
if ($__bag->has($__errorArgs[0])) :
if (isset($message)) { $__messageOriginal = $message; }
$message = $__bag->first($__errorArgs[0]); ?> <span class="text-red-500 text-sm"><?php echo e($message); ?></span> <?php unset($message);
...
<?php /**PATH C:\Users\Tolga\Desktop\Proje Siteleri\linkkısaltmaservisi2\resources\views/livewire/user/links.blade.php ENDPATH**/ ?>