    ...
    <div class="bg-gradient-to-br from-blue-900/30 to-purple-900/30 rounded-xl p-5 border border-blue-500/20">
        <div class="flex items-center justify-between mb-4">
            <h3 class="text-lg font-bold text-white flex items-center gap-2">
                <span class="text-2xl">🎯</span> Günlük Görevler
            </h3>
                ...
                <div class="bg-gray-800/50 rounded-lg p-3 {{ $isCompleted ? 'border border-green-500/30' : '' }}">
                    ...
                    <div class="mb-1">
                        <div class="flex justify-between text-xs text-gray-400 mb-1">
                            ...
                            <span class="text-yellow-400">+{{ $challenge['points_reward'] }} puan</span>
                        </div>
            ...
            @if($todaysChallenges->isAllCompleted())
                @if($todaysChallenges->bonus_claimed)
                    <div class="flex items-center justify-center gap-2 text-green-400">
                        <span>🎉</span>
                        <span>3/3 Bonus alındı! (+{{ $bonusPoints }} puan)</span>
                    </div>
                @else
                    <button 
                    ...
                    >
                        ...
                        3/3 Bonus Al (+{{ $bonusPoints }} Puan)
                    </button>
                @endif
            @else
                <div class="text-center text-gray-400 text-sm">
                    Tüm görevleri tamamlayın ve <span class="text-yellow-400 font-semibold">+{{ $bonusPoints }} bonus puan</span> kazanın!
                </div>
            @endif
        </div>
    </div>
