<footer class="bg-black border-t border-gray-800 py-20 px-4 sm:px-6 lg:px-8">
    <div class="max-w-7xl mx-auto">
        <div class="grid grid-cols-1 md:grid-cols-4 gap-12 mb-12">
            ...
            <div class="col-span-1">
                ...
                <div class="flex gap-4">
                    ...
                    <a href="{{ setting('facebook_url') }}" target="_blank" rel="noopener noreferrer" class="w-11 h-11 rounded-full bg-gray-900 border border-gray-800 flex items-center justify-center text-gray-400 hover:text-electric-blue hover:border-electric-blue/50 transition-all duration-300">
                        <svg class="w-5 h-5" fill="currentColor" viewBox="0 0 24 24"><path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z"/></svg>
                    </a>
                    @endif
                    @if(setting('twitter_url'))
                    <a href="{{ setting('twitter_url') }}" target="_blank" rel="noopener noreferrer" class="w-11 h-11 rounded-full bg-gray-900 border border-gray-800 flex items-center justify-center text-gray-400 hover:text-electric-blue hover:border-electric-blue/50 transition-all duration-300">
                        <svg class="w-5 h-5" fill="currentColor" viewBox="0 0 24 24"><path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-5.214-6.817L4.99 21.75H1.68l7.73-8.835L1.254 2.25H8.08l4.713 6.231zm-1.161 17.52h1.833L7.084 4.126H5.117z"/></svg>
                    </a>
                    @endif
                    @if(setting('instagram_url'))
                    <a href="{{ setting('instagram_url') }}" target="_blank" rel="noopener noreferrer" class="w-11 h-11 rounded-full bg-gray-900 border border-gray-800 flex items-center justify-center text-gray-400 hover:text-electric-blue hover:border-electric-blue/50 transition-all duration-300">
                    ...
                    <a href="{{ setting('youtube_url') }}" target="_blank" rel="noopener noreferrer" class="w-11 h-11 rounded-full bg-gray-900 border border-gray-800 flex items-center justify-center text-gray-400 hover:text-electric-blue hover:border-electric-blue/50 transition-all duration-300">
                        <svg class="w-5 h-5" fill="currentColor" viewBox="0 0 24 24"><path d="M23.498 6.186a3.016 3.016 0 0 0-2.122-2.136C19.505 3.545 12 3.545 12 3.545s-7.505 0-9.377.505A3.017 3.017 0 0 0 .502 6.186C0 8.07 0 12 0 12s0 3.93.502 5.814a3.016 3.016 0 0 0 2.122 2.136c1.871.505 9.376.505 9.376.505s7.505 0 9.377-.505a3.015 3.015 0 0 0 2.122-2.136C24 15.93 24 12 24 12s0-3.93-.502-5.814zM9.545 15.568V8.432L15.818 12l-6.273 3.568z"/></svg>
                    </a>
                    @endif
                    @if(setting('linkedin_url'))
                    <a href="{{ setting('linkedin_url') }}" target="_blank" rel="noopener noreferrer" class="w-11 h-11 rounded-full bg-gray-900 border border-gray-800 flex items-center justify-center text-gray-400 hover:text-electric-blue hover:border-electric-blue/50 transition-all duration-300">
            ...
            <div>
                ...
                <ul class="space-y-4 text-base text-gray-400" style="font-family: 'Inter', 'Space Grotesk', sans-serif;">
                    ...
                    <li><a href="{{ route('blog.index') }}" class="hover:text-electric-blue transition-colors flex items-center gap-2">
            ...
            <!-- Stats Column -->
            @if(setting('display_homepage_stats', true))
            <div>
                ...
                <ul class="space-y-4 text-base text-gray-400" style="font-family: 'Inter', 'Space Grotesk', sans-serif;">
                    ...
                    <li class="flex items-center gap-2">
                        ...
                        <span class="text-white font-semibold">{{ number_format($realUsers + $fakeUsers) }}</span> Publishers
                    </li>
                    <li class="flex items-center gap-2">
                        <span class="material-symbols-outlined text-lg text-bright-magenta">link</span>
                        <span class="text-white font-semibold">{{ number_format($realLinks + $fakeLinks) }}</span> Links
                    </li>
                    <li class="flex items-center gap-2">
                        <span class="material-symbols-outlined text-lg text-electric-blue">touch_app</span>
                        <span class="text-white font-semibold">{{ number_format($realClicks + $fakeClicks) }}</span> Clicks
                    </li>
                </ul>
            </div>
            @endif
        ...
        <div class="border-t border-gray-900 pt-8 flex flex-col md:flex-row justify-between items-center gap-4">
            <p class="text-gray-500 text-base" style="font-family: 'Inter', 'Space Grotesk', sans-serif;">© {{ date('Y') }} {{ config('app.name', 'MrShort') }}. All rights reserved.</p>
