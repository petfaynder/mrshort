<!DOCTYPE html>
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}" class="{{ auth()->user()->theme_preference ?? 'dark' }}">
<head>
    <meta charset="utf-8">
    <script>
    ...
    <div class="h-screen flex flex-col">
                ...
                <div class="flex justify-between h-14">
                    <div class="flex">
                        ...
                        <div class="shrink-0 flex items-center">
                            <a href="{{ route('feedback.index') }}" class="flex items-center gap-2 group">
                    ...
                    <div class="flex items-center gap-3">
                        ...
                        <a href="{{ route('dashboard') }}" class="btn-spin text-sm text-text-light hover:text-heading-light dark:text-text-dark dark:hover:text-heading-dark flex items-center gap-1">
                        ...
                        @auth
                            <div class="flex items-center gap-2 ml-2">
                                <img alt="User avatar" class="w-8 h-8 rounded-full" src="https://ui-avatars.com/api/?name={{ urlencode(auth()->user()->name) }}&size=32&background=0D8ABC&color=fff"/>
                                <span class="text-sm font-medium text-heading-light dark:text-heading-dark">{{ auth()->user()->name }}</span>
                            </div>
                        @else
                            <a href="{{ route('login') }}" class="text-sm font-medium text-primary hover:text-blue-700">Log in</a>
                            <a href="{{ route('register') }}" class="ml-2 text-sm font-medium text-text-light hover:text-heading-light dark:text-text-dark">Register</a>
                        @endauth
                    </div>
                </div>
            </div>
        ...
        <footer class="bg-card-light dark:bg-card-dark border-t border-border-light dark:border-border-dark py-4 shrink-0">
            <div class="max-w-7xl mx-auto px-4 text-center text-sm text-text-light dark:text-text-dark">
                &copy; {{ date('Y') }} MRShort. All rights reserved.
            </div>
        </footer>
    </div>
